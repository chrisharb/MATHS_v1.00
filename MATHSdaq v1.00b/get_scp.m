function [scope,sn,sn_c,nscp] = get_scp(library)
...
if library.DeviceList.Count == 0 %Check if MATLAB detected any devices
    ...
    sn_c = -1;
    nscp = -1;
    disp('No oscilloscopes found. If connection problem persists remove and reinsert device')
elseif ismac %If computer runs Apple IOS this code will not work :-(
    scope = -1;
    sn = -1;
    nscp = -1;
    sn_c = -1;
    disp('Cannot run on Mac. Bye.')
    ...
    if length(scps) > 1
        ...
        clear scps;
        disp(['I found ' num2str(length(scope.Channels)) ' available channels across ' num2str(nscp) ' oscilloscopes, which are now combined.']);
    else
        scope = scps;
        sn_c = scope.SerialNumber;
        clear scps;
        disp(['I found ' num2str(length(scope.Channels)) ' available channels which is a standalone instrument']);
    end
end
clear item
